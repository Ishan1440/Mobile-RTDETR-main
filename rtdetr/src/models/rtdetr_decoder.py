...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(
                output, ref_points_input, memory,
...
class RTDETRTransformer(nn.Module):
    ...
    def _get_decoder_input(
        ...
        # print((enc_outputs_class.max(1).values).shape, self.num_queries)
        ...
        enc_topk_logits = enc_outputs_class.gather(
            ...
            .repeat(1, 1, enc_outputs_class.shape[-1])
        )
        ...
        if self.learnt_init_query:
            ...
            target = output_memory.gather(
                dim=1,
                index=topk_ind.unsqueeze(-1)
                .repeat(1, 1, output_memory.shape[-1])
            )
