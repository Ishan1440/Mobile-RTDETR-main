...
def get_contrastive_denoising_training_group(
    ...
    pad_gt_mask = torch.zeros(
        [bs, max_gt_num], dtype=torch.bool, device=device)
    ...
    dn_positive_idx = torch.split(
        dn_positive_idx, [n * num_group for n in num_gts])
    ...
    if box_noise_scale > 0:
        ...
        diff = torch.tile(
            input_query_bbox[..., 2:] * 0.5, [1, 1, 2]) * box_noise_scale
    ...
    attn_mask = torch.full([tgt_size, tgt_size], False,
    ...
    for i in range(num_group):
        if i == 0:
            attn_mask[max_gt_num * 2 * i: max_gt_num * 2 *
                      (i + 1), max_gt_num * 2 * (i + 1): num_denoising] = True
        if i == num_group - 1:
            attn_mask[max_gt_num * 2 * i: max_gt_num *
                      2 * (i + 1), :max_gt_num * i * 2] = True
        else:
            attn_mask[max_gt_num * 2 * i: max_gt_num * 2 *
                      (i + 1), max_gt_num * 2 * (i + 1): num_denoising] = True
            attn_mask[max_gt_num * 2 * i: max_gt_num *
                      2 * (i + 1), :max_gt_num * 2 * i] = True
